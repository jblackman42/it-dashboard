...
const Dashboard = () => {
  ...
  const daysBackArray = useMemo(() => [...Array(daysBack)].map((_, i) => i).reverse().map(i => {
    const date = new Date();
  ...
  useEffect(() => {
    if (initialized.current && tickets.length > 0) {
      ...
      [...new Set(tickets.filter(ticket => ticket.Tag).map(ticket => ticket.Tag))].sort().forEach(tag => {
        pushToArray(setTicketTagLabels, tag);
        pushToArray(setTicketTagValues, tickets.filter(ticket => new Date(ticket.Request_Date) > oldestDate && ticket.Tag === tag).length);
      });
      ...
      setCurrentOpenedTickets(tickets.filter(ticket => ticket.Status_ID !== 4 && ticket.Status_ID !== 3 && ticket.Request_Method_ID !== 5).length);
      ...
      const ticketResponseTimes = tickets.filter(ticket => ticket._First_Response_Date !== null).map(ticket => {
      ...
      const avgResponseTime = (ticketResponseTimes.reduce((accum, val) => accum + val, 0) / ticketResponseTimes.length) / 3600000;
      const avgResponseTimeString = `${Math.floor(avgResponseTime)}hrs ${Math.round((avgResponseTime - Math.floor(avgResponseTime)) * 60)}m`
  ...
  return (
    <article id="dashboard">
      <div className="row">
        <button onClick={refreshData} disabled={isRefreshing}>Refresh</button>
      </div>
      <div className="row">
        ...
        <div className="column">
          <DashChart type="KPI" title="Current Opened Tickets" values={[currentOpenedTickets]}/>
          <DashChart type="KPI" title="Average Ticket Response Time" values={[averageTicketResponseTime]}/>
        </div>
        <div className="column">
      ...
      <div className="row">
        ...
        <div className="column">
          <DashChart type="Doughnut" title={`Ticket By Tag - (Last ${daysBack} Days)`} units={["Tickets"]} labels={ticketTagLabels} values={[ticketTagValues]} />
        </div>
        <div className="column">
